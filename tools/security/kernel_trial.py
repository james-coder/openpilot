...
ROOT = Path('/usr/local/lib/comma-kernel-trial')
...
TIMER = 'comma-kernel-rollback.timer'
UNIT = 'comma-kernel-rollback.service'
SERVICE_TEXT = '''[Unit]
Description=Guarded rollback of unconfirmed experimental kernel
After=local-fs.target
[Service]
Type=oneshot
ExecStart=/usr/bin/python3 -I /usr/local/lib/comma-kernel-trial/kernel_trial.py rollback
Restart=on-failure
RestartSec=30
TimeoutStartSec=120
'''
...
def rollback():
  current = phase()
  if current == 'restored' and Path('/sys/firmware/devicetree/base/soc/ssusb@a800000/comma,untrusted-modem-host').exists():
    hardware()
...
def install(source, expected):
  ...
  if shutil.disk_usage('/').free < 192 * 1024 * 1024:
  ...
  with writable():
    ...
    shutil.copyfile(Path(__file__), ROOT / 'kernel_trial.py')
    (ROOT / 'manifest.json').write_text(json.dumps(dict(candidate_sha256=expected,
      partition_sha256=hashlib.sha256(candidate + baseline[len(candidate):]).hexdigest())))
...
if __name__ == '__main__':
  ...
  if args.action == 'install':
    if args.source is None or args.sha256 is None:
      parser.error('install requires --source and --sha256')
    install(args.source, args.sha256)
  else:
    trusted(ROOT / 'kernel_trial.py')
    fd = os.open('/run/comma-kernel-trial.lock', os.O_CREAT | os.O_RDWR | os.O_NOFOLLOW, 0o600)
