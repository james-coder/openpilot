...
void vgw_led_rx_health(uint8_t status[3], bool tx_inhibited) {
  if (status && tx_inhibited && !status[2] &&
      (status[0]==VGW_LED_RUNNING || status[0]==VGW_LED_TRIAL)) status[0]=VGW_LED_RX_DEGRADED;
}
void vgw_led_set(vgw_status_led *s, unsigned state, unsigned slot, unsigned error, uint32_t now) {
  ...
  bool valid = state <= VGW_LED_RX_DEGRADED && slot_valid && (state != VGW_LED_PROBE || slot == 255) &&
...
uint8_t vgw_led_sample(vgw_status_led *s, uint32_t now) {
  ...
  switch (s->state) {
    ...
    case VGW_LED_RX_DEGRADED: {
      /* One long red, dark separator, short blue, long dark separator.
       * Distinct from counted fatal red codes; no sleeps or busy waiting. */
      uint32_t phase=elapsed%6000U;
      return phase<1500U ? VGW_LED_RED : phase>=2000U && phase<2250U ? VGW_LED_BLUE : 0;
    }
