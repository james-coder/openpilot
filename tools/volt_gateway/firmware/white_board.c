...
#define RAM __attribute__((section(".ramfunc.vgw_board"), noinline))

static RAM bool valid(const vgw_white_mmio *io) { return io && io->read32 && io->read16 && io->write32; }
static RAM void change(const vgw_white_mmio *io, uint32_t reg, uint32_t clear, uint32_t set) {
  io->write32(io->ctx, reg, (io->read32(io->ctx, reg) & ~clear) | set);
}
static RAM uint32_t pairs(uint32_t pins) {
  uint32_t mask=0;
  for (unsigned i=0; i<16; i++) if (pins & (1U << i)) mask |= 3U << (2*i);
  return mask;
}
static RAM void outputs(const vgw_white_mmio *io, uint32_t port, uint32_t pins) {
...
static RAM void inputs(const vgw_white_mmio *io, uint32_t port, uint32_t pins) {
  change(io, port+MODER, pairs(pins), 0);
  change(io, port+PUPDR, pairs(pins), 0);
}
static RAM bool verify_pins(const vgw_white_mmio *io, uint32_t port, uint32_t pins, bool output) {
...
bool vgw_white_identify(const vgw_white_mmio *io, vgw_white_identity *out) {
  ...
  *out=(vgw_white_identity){(uint16_t)(id & 0xfffU), (uint16_t)(id >> 16), io->read16(io->ctx, 0x1fff7a22U)};
  ...
  return out->device_id == 0x463 && out->flash_kib == VGW_FLASH_KIB;
}
RAM bool vgw_white_quiesce(const vgw_white_mmio *io) {
